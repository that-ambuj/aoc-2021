...
fn part1_solution(output_values: &[&str]) -> usize {
    output_values
        .par_iter()
...
fn part2_solution(input: &[(&str, &str)]) -> u32 {
    let answer = input
        .iter()
        .map(|(signal, output)| {
...
fn decode_signal(values: &[HashSet<char>]) -> Vec<(&HashSet<char>, u8)> {
    ...
    let number_1 = decoded.iter().find(|(_, n)| *n == 1).unwrap().0.clone();
    let number_4 = decoded.iter().find(|(_, n)| *n == 4).unwrap().0.clone();
